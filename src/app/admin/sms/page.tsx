// =====================================================
// The Hub - SMS Page (إرسال رسائل SMS)
// =====================================================

'use client';

import { useState, useEffect } from 'react';
import { MessageSquare, Send, Users, Check } from 'lucide-react';
...
export default function SMSPage() {
    ...
    useEffect(() => {
        supabase.from('users').select('*').eq('role', 'user').not('phone', 'is', null).then(({ data }) => setUsers(data || []));
    }, []);
    ...
    async function handleSend(e: React.FormEvent) {
        ...
        // TODO: إرسال SMS عبر API Route باستخدام Twilio
        // const phones = users.filter(u => selectedUsers.includes(u.id)).map(u => u.phone);
        // await fetch('/api/send-sms', { method: 'POST', body: JSON.stringify({ phones, message }) });

        await new Promise(r => setTimeout(r, 1000));
        setLoading(false);
        setSent(true);
        setTimeout(() => { setSent(false); setMessage(''); setSelectedUsers([]); setSelectAll(false); }, 2000);
    }
    ...
    return (
        <div className="space-y-6">
            <h1 className="text-2xl font-bold">إرسال رسائل SMS</h1>
            ...
            <form onSubmit={handleSend} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="card">
                    <h3 className="font-bold text-lg mb-4 flex items-center gap-2"><MessageSquare className="w-5 h-5" />محتوى الرسالة</h3>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm mb-2">الرسالة</label>
                            <textarea required value={message} onChange={e => setMessage(e.target.value)} className="input" rows={6} placeholder="اكتب رسالتك هنا..." />
                            <div className="flex justify-between text-sm text-workspace-muted mt-2">
